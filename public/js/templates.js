...
(function() {
  ...
  Templates.renderPost = post => {
    ...
    switch (post.api) {
      ...
      default:
        throw new Error('Invalid API: ' + post.api)
    }
   ...
   */
  Templates.renderSearchResults = results => {
    if (results.length === 0) {
      return tag('span', {}, 'No results found.')
    }

    return tag('div', {}, results.map(result => {
      return tag('div', {class: 'result'}, [
        tag('img', {alt: result.api, src: 'images/' + result.api + '.png',}, []),
